...
this.boxes[userId]=new RSChatLayout(id,name);RSConfig.beforeDialogOpen();this.boxes[userId].init('opened');RSChat.reInitWindow()},newMessage:function(data){var userId='user_'+data.userId;this.start(userId,data.name);this.boxes[userId].createMsg(data)},remove:function(id){var userId='user_'+id;if(this.boxes.hasOwnProperty(userId)){delete this.boxes[userId];return!0}},reInitWindow:function(){var _=this;var li=null;var marginRight=0;var maxWindow=parseInt((window.innerWidth-100)/(RSConfig.width+RSConfig.marginRight));RSConfig.hiddenContainer.style.display='none';var items=Object.keys(RSChat.boxes).reverse();if(items.length<maxWindow){maxWindow=items.length}
